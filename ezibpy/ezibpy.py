...
class ezIBpy():
    ...
    def __init__(self):
        ...
        #state variables to enable printing of a whole content
        self.track = ["AccountType", "AccountCode", "AccountReady", "AvailableFunds","NetLiquidationByCurrency"]
        self.accountState = 0
        self.stkState = 0
        self.accountUpdateReadyMask = 0
        self.stkStateReadyMask = 0
        for i, name in enumerate(self.track):
            self.accountUpdateReadyMask |= (1<<i);
    ...
    def handleAccount(self, msg):
        ...
        """
        #print("acct update called ", msg)
        if msg.key in self.track:
            # log handler msg
            self.log_msg("account", msg)
            # assign value
            self.account[msg.key] = str(msg.value)
            #print("account key {}, account value {}", msg.key, msg.value)
            self.accountState |= (1 << self.track.index(msg.key))

        #print("account ready mask is ", self.accountUpdateReadyMask)
        #print("account state is ",self.accountState)
        if self.accountUpdateReadyMask == self.accountState:
            self.accountState = 0
            tablemeta = list(self.account.keys())
            tablecontent = list(self.account.values())
            table = [tablemeta, tablecontent]
            if(len(tablemeta) > 0 and len(tablecontent)>0):
                # fire callback
                self.ibCallback(caller="handleAccount", msg=msg, table=table)
            else:
                self.ibCallback(caller="error", msg="table data is not fullfilled correctly")
    ...
    def handleTickPrice(self, msg):
        ...
        print("msg ticker id {}", msg.tickerId)
        print("msg ticker price {}", msg.price)
        print("msg ticker field {}", msg.field)
